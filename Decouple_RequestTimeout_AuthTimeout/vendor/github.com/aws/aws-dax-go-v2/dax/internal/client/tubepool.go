...
func (p *tubePool) getWithContext(ctx context.Context, highPriority bool, opt RequestOptions) (tube, error) {
	for {
		...
		if p.top != nil {
			...
			t.SetNext(nil)
			p.mutex.Unlock()
			fmt.Println("[tubepool::getWithContext] The client successfully retrieved a connection from the pool.")
			return t, nil
		}
		...
		case tube := <-waitCh:
			if tube != nil {
				fmt.Println("[tubepool::getWithContext] The client received an idle tube, via waiting channel, after no free one was found in tubepool.")
				return tube, nil
			}
			// if channel is closed, continue to look for idle tubes in stack
		case tube := <-done:
			if tube != nil {
				fmt.Println("[tubepool::getWithContext] The client allocated a new tube, as no free one was found in tubepool.")
				return tube, nil
			}
		case err := <-p.errCh:
		...
		case <-ctx.Done():
			...
			fmt.Println("[tubepool::getWithContext] The context deadline expired, before allocating new tube/returning idle tube.")
			return nil, ctx.Err()
		}
	}
}
...
func (p *tubePool) put(t tube) {
	...
	if p.waiters != nil {
		select {
		case p.waiters <- t:
			fmt.Println("[tubepool::put] Send the input idle tube, which was previously allocated, to a waiter, instead of adding it to tubepool.")
			return
		default:
	...
	fmt.Println("[tubepool::put] Add the input idle tube, which was previously allocated, to the tubepool stack")
	t.SetNext(p.top)
	p.top = t
}
...
func (p *tubePool) setDeadline(ctx context.Context, tube tube) error {
	select {
	case <-ctx.Done():
		fmt.Println("[tubepool::setDeadline] The context deadline expired, before setting deadline to the input tube.")
		return ctx.Err()
	default:
	}
	var deadline time.Time
	if d, ok := ctx.Deadline(); ok {
		fmt.Println("[tubepool::setDeadline] The client sets deadline:", d.String())
		deadline = d
	}
...
func (p *tubePool) debugLog(opt RequestOptions, logString string, args ...interface{}) {
